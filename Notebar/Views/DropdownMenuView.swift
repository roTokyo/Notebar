...
struct DropdownMenuView: NSViewRepresentable {
    ...
    func updateNSView(_ nsView: NSPopUpButton, context: Context) {
        ...
        let shortcutItem = NSMenuItem(title: "Imposta scorciatoia (\(shortcutManager.displayString))…", action: #selector(Coordinator.shortcutAction), keyEquivalent: "")
        shortcutItem.target = context.coordinator
        ...
        nsView.menu?.insertItem(shortcutItem, at: 1)
    ...
    class Coordinator: NSObject {
        ...
        @objc func shortcutAction(_ sender: NSMenuItem) {
            (NSApp.delegate as? AppDelegate)?.startRecordingShortcut()
        }
    }
}
